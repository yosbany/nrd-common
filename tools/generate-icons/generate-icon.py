#!/usr/bin/env python3
"""
Generador de iconos SVG y PNG para proyectos NRD
Genera iconos personalizados con el texto proporcionado
Uso: python3 generate-icon.py "TEXTO_DEL_ICONO" [proyecto]
"""
...
def main():
    if len(sys.argv) < 2:
        ...
        print("   Uso: python3 generate-icon.py \"TEXTO_DEL_ICONO\" [proyecto]")
        return 1
    ...
    if project_name:
        ...
        if 'nrd-' in str(current_dir):
            ...
            # Estamos en common, necesitamos el nombre del proyecto
            print("✗ Error: No se especificó el proyecto y no se puede detectar automáticamente")
            print("   Uso: python3 generate-icon.py \"TEXTO_DEL_ICONO\" [proyecto]")
            print("   Ejemplo: python3 generate-icon.py \"NRD RRHH\" nrd-rrhh")
            return 1
    
    if not project_root.exists():
        print(f"✗ Error: Proyecto '{project_root}' no encontrado")
        return 1
    ...
    try:
        ...
        # Crear directorio assets/icons si no existe
        assets_dir = project_root / "assets" / "icons"
        assets_dir.mkdir(parents=True, exist_ok=True)
        
        png_192_path = assets_dir / "icon-192.png"
        if not convert_svg_to_png_from_file(svg_192_path, png_192_path, 192):
            return 1
        
        png_512_path = assets_dir / "icon-512.png"
